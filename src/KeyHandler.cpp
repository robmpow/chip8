...
std::ostream& operator << (std::ostream& t_os, const KeyPair &t_keyPair){
    t_os << SDL_GetKeyName(t_keyPair.key) << ":" << KeyHandler::getNameFromKmod(t_keyPair.modifier);
    return t_os;
}

std::string KeyHandler::getNameFromKmod(uint16_t t_modifiers){
...
uint16_t KeyHandler::getKmodFromName(const std::string& t_modName){
...
std::string KeyHandler::getNameFromAction(KeyAction t_keyAction){
...
KeyAction KeyHandler::getActionFromName(const std::string& t_actionName){
...
void KeyHandler::bindKey(KeyPair t_keyPair, KeyAction t_keyAction){
    ...
    if(res != m_bindMap.end()){
        chip8Logger.log<Logger::LogTrace>("Chip8::KeyHandler::bindKey, Replaced key binding, Old Binding: '", res->first, "'->'", KeyHandler::getNameFromAction(res->second),"'; New Binding: '", res->first, "'->'", KeyHandler::getNameFromAction(t_keyAction), "'", Logger::endl);
        res->second = t_keyAction;
    }
    else{
        chip8Logger.log<Logger::LogTrace>("Chip8::KeyHandler::bindKey, New key binding: '", t_keyPair, "'->'", KeyHandler::getNameFromAction(t_keyAction), "'", Logger::endl);
...
void KeyHandler::bindAction(KeyAction t_keyAction, std::function<void(bool, bool)> t_context){
    if(m_handlerContext[t_keyAction]){
        chip8Logger.log<Logger::LogTrace>("Chip8::KeyHandler::bindAction, Replaced context bind for '", KeyHandler::getNameFromAction(t_keyAction), "'", Logger::endl);
    }
    chip8Logger.log<Logger::LogTrace>("Chip8::KeyHandler::bindAction, New context bind for '", KeyHandler::getNameFromAction(t_keyAction), "'", Logger::endl);
...
} // namespace Chip8 namespace KeyHandler